...
const fileUploadValidation = Joi.object({
    file: Joi.object({
        mime: Joi.string().valid('image/jpeg', 'image/png', 'image/jpg', 'application/pdf'),
        size: Joi.number().max(1024 * 1024 * 5).label('File size (up to 5MB)'),
    }),
...
export { fileUploadValidation, fileIsExist, createFileUrl }