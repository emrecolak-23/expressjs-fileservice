...
class ConsulInstance {
    ...
    private consulService: { name: string; id: string, address: string, check: any, port: number };
    ...
    constructor(service: string, uri: string) {
        ...
        console.log(address)
        let healthCheckUrl = "http://localhost:3001/file-service/healthcheck";
        if (process.env.NODE_ENV === "production") {
            healthCheckUrl = `http://${address}:3000/file-service/healthcheck`
        }

        console.log('---------eth0 ip :', address)
        this.consulService = {
            name: service,
    ...
    getConsulService(): { name: string; id: string, address: string, port: number, check: any } {
